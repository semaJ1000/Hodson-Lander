...
/**
BoundingBox *GLMmodel::calcBoundingBox(BoundingBox *box)
{
    ...
    {
	if (i == 0) {
		m_BoundingBox.xMin = m_BoundingBox.xMax = vertices[3*i+0];
		m_BoundingBox.yMin = m_BoundingBox.yMax = vertices[3*i+1];
		m_BoundingBox.zMin = m_BoundingBox.zMax = vertices[3*i+2];
	}
	else {
	if( vertices[3*i+0] < m_BoundingBox.xMin )
	    m_BoundingBox.xMin = vertices[3*i+0] ;
	if( vertices[3*i+1] < m_BoundingBox.yMin )
	    m_BoundingBox.yMin = vertices[3*i+1] ;
	if( vertices[3*i+2] < m_BoundingBox.zMin )
	    m_BoundingBox.zMin = vertices[3*i+2] ;

	if( vertices[3*i+0] > m_BoundingBox.xMax )
	    m_BoundingBox.xMax = vertices[3*i+0] ;
	if( vertices[3*i+1] > m_BoundingBox.yMax )
	    m_BoundingBox.yMax = vertices[3*i+1] ;
	if( vertices[3*i+2] > m_BoundingBox.zMax )
	    m_BoundingBox.zMax = vertices[3*i+2] ;
		}
	}
...
glmFindMaterial(GLMmodel* model, char* name)
{
    ...
    material (0). */
    PrintMessage("glmFindMaterial():  can't find material \"%s\".\n", name);
    i = 0;
...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    if (!file) {
        PrintMessage( "glmReadMTL() failed: can't open material file \"%s\".\n",
            filename);
        exit(1);
    }
    free(filename);
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'n':               /* newmtl */
			...
			model->materials[nummaterials].tex = NULL;
            break;
        case 'N':
        ...
        case 'm':
			...
			char filename[128];
		    dir = glmDirName(model->pathname);
			sprintf( filename , "%s%s", dir,buf );
			free(dir);
			model->materials[nummaterials].tex = new Texture( filename, true );
			model->materials[nummaterials].tex->ChangeSettings(GL_LINEAR,GL_LINEAR_MIPMAP_LINEAR,GL_REPEAT,GL_REPEAT,GL_MODULATE);
			break;
        case 'K':
            switch(buf[1]) {
            case 'd':
...
/*        case 'm':
            fgets(buf, sizeof(buf), file);
            sscanf(buf, "%s %s", buf, buf);
			model->materials[nummaterials].tex = new Texture ( buf );
			model->materials[nummaterials].tex->ChangeSettings( GL_LINEAR, GL_LINEAR_MIPMAP_NEAREST, GL_REPEAT, GL_REPEAT, GL_MODULATE);
			break;
  */      default:
            /* eat up rest of line */
            fgets(buf, sizeof(buf), file);
...
glmWriteMTL(GLMmodel* model, char* modelpath, char* mtllibname)
{
    ...
    if (!file) {
        PrintMessage( "glmWriteMTL() failed: can't open file \"%s\".\n",
            filename);
        exit(1);
    }
    free(filename);
...
#endif
    scale = 1.0f / glmMax(glmMax(w, h), d);
 ...
 */
GLvoid
glmScale(GLMmodel* model, GLfloat x, GLfloat y, GLfloat z)
{
    GLuint i;
...
glmVertexNormals(GLMmodel* model, GLfloat angle)
{
    ...
    /* calculate an average normal for this vertex by averaging the
        ...
        node = members[i];
        if (!node)
            PrintMessage( "glmVertexNormals(): vertex w/o a triangle\n");
...
#if 0
    PrintMessage("glmLinearTexture(): generated %d linear texture coordinates\n",
        model->numtexcoords);
#endif
}
...
/* ~GLMmodel: Deletes a GLMmodel structure.
 */
GLMmodel::~GLMmodel()
{
    GLMgroup* group;
...
//** Use only if specific case, such as blenshapes
int GLMmodel::PartialCopy(GLMmodel *src)
{
	this->pathname = _strdup(src->pathname) ;
    this->mtllibname    =  _strdup(src->mtllibname) ;

	if( src->GetNumVertices() > 0 )
	{
		int n = src->GetNumVertices()+1 ;
		this->numvertices   =  n-1 ;
		this->vertices = (GLfloat *) malloc(n*3*sizeof(GLfloat));
		if( this->vertices == NULL )
		{
			PrintMessage("GLMmodel: Error: cannot allocate memory for vertices.") ;
			return A_ERROR;
		}
		memcpy(this->vertices,src->vertices,n*3*sizeof(GLfloat)) ;
	}
	if( src->GetNumNormals() > 0 )
	{
		int n = src->GetNumNormals()+1 ;
		this->numnormals   = n-1 ;
		this->normals = (GLfloat *) malloc(n*3*sizeof(GLfloat));
		if( this->normals == NULL )
		{
			PrintMessage("GLMmodel: Error: cannot allocate memory for normals.") ;
			return A_ERROR;
		}
		memcpy(this->normals,src->normals,n*3*sizeof(GLfloat)) ;
	}
	if( src->GetNumTexCoords() > 0 )
	{
		int n = src->GetNumTexCoords()+1 ;
		this->numtexcoords   = n-1;
		this->texcoords = (GLfloat *) malloc(n*2*sizeof(GLfloat));
		if( this->texcoords == NULL )
		{
			PrintMessage("GLMmodel: Error: cannot allocate memory for texture coordinates.") ;
			return A_ERROR ;
		}
		memcpy(this->texcoords,src->texcoords,n*2*sizeof(GLfloat)) ;
	}

	if( src->numfacetnorms > 0 )
	{
		int n = src->numfacetnorms + 1;
		this->numfacetnorms   = n-1 ; 
		this->facetnorms = (GLfloat *) malloc(n*3*sizeof(GLfloat));
		if( this->facetnorms == NULL )
		{
			PrintMessage("GLMmodel: Error: cannot allocate memory for facet normals.") ;
			return A_ERROR;
		}
		memcpy(this->facetnorms,src->facetnorms,n*3*sizeof(GLfloat)) ;
	}
 ...
 */
int
GLMmodel::ReadOBJ(const char* filename)
{
    FILE*   file;
	...
	if( filename == NULL ) 
	{
		PrintMessage("ReadOBJ() failed: got a null pointer.") ;
		return A_ERROR ;
	}
    file = fopen(filename, "r");
    if (!file) {
		PrintMessage("ReadOBJ() failed: can't open data file \"%s\".\n",
            filename);
        return A_ERROR;
    }
    
    /* allocate a new model */
    this->pathname    = _strdup(filename);
        
    /* make a first pass through the file to get a count of the number
    of vertices, normals, texcoords & triangles */
    glmFirstPass(this, file);
...
glmWriteOBJ(GLMmodel* model, char* filename, GLuint mode)
{
    ...
    if (mode & GLM_FLAT && !model->facetnorms) {
        PrintMessage("glmWriteOBJ() warning: flat normal output requested "
            "with no facet normals defined.\n");
        mode &= ~GLM_FLAT;
    }
    if (mode & GLM_SMOOTH && !model->normals) {
        PrintMessage("glmWriteOBJ() warning: smooth normal output requested "
            "with no normals defined.\n");
        mode &= ~GLM_SMOOTH;
    }
    if (mode & GLM_TEXTURE && !model->texcoords) {
        PrintMessage("glmWriteOBJ() warning: texture coordinate output requested "
    ...
    if (mode & GLM_COLOR && !model->materials) {
        PrintMessage("glmWriteOBJ() warning: color output requested "
            "with no colors (materials) defined.\n");
        mode &= ~GLM_COLOR;
    }
    if (mode & GLM_MATERIAL && !model->materials) {
        PrintMessage("glmWriteOBJ() warning: material output requested "
            "with no materials defined.\n");
        mode &= ~GLM_MATERIAL;
    }
    if (mode & GLM_COLOR && mode & GLM_MATERIAL) {
        PrintMessage("glmWriteOBJ() warning: color and material output requested "
    ...
    file = fopen(filename, "w");
    if (!file) {
		PrintMessage( "glmWriteOBJ() failed: can't open file \"%s\" to write.\n",
		    filename);
        exit(1);
    }
...
glmDraw(GLMmodel* model, GLuint mode)
{
	...
	if (mode & GLM_FLAT && !model->facetnorms) {
        PrintMessage("glmDraw() warning: flat render mode requested "
            "with no facet normals defined.\n");
        mode &= ~GLM_FLAT;
    }
    if (mode & GLM_SMOOTH && !model->normals) {
        PrintMessage("glmDraw() warning: smooth render mode requested "
            "with no normals defined.\n");
        mode &= ~GLM_SMOOTH;
    }
    if (mode & GLM_TEXTURE && !model->texcoords) {
        PrintMessage("glmDraw() warning: texture render mode requested "
            "with no texture coordinates defined.\n");
        mode &= ~GLM_TEXTURE;
    }
    if (mode & GLM_FLAT && mode & GLM_SMOOTH) {
    ...
    if (mode & GLM_COLOR && !model->materials) {
        PrintMessage("glmDraw() warning: color render mode requested "
            "with no materials defined.\n");
        mode &= ~GLM_COLOR;
    }
    if (mode & GLM_MATERIAL && !model->materials) {
        PrintMessage("glmDraw() warning: material render mode requested "
            "with no materials defined.\n");
        mode &= ~GLM_MATERIAL;
    }
    if (mode & GLM_COLOR && mode & GLM_MATERIAL) {
        PrintMessage("glmDraw() warning: color and material render mode requested "
    ...
    while (group)
	{
        if (mode & GLM_MATERIAL)
		{
			...
			if (mode & GLM_TEXTURE && material->tex != NULL )
				material->tex->MakeCurrent();
        }
...
#if 0
    PrintMessage("glmWeld(): %d redundant vertices.\n", 
...
glmReadPPM(const char* filename, int* width, int* height)
{
    ...
    if (strncmp(head, "P6", 2)) {
        PrintMessage( "%s: Not a raw PPM file\n", filename);
        return NULL;
    }
...
if (model->numnormals) {
    ...
    PrintMessage("glmOptimize(): %d redundant normals.\n", 
...
if (model->numtexcoords) {
    ...
    PrintMessage("glmOptimize(): %d redundant texcoords.\n", 
