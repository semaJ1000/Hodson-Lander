...
void SpecialKeyboardCB(int key, int x, int y) {
    ...
    // Then try to update the lander if it exists
    if (BaseSimulator* simulator = GlobalResourceManager::use()->getSimulator("landerSim")) {
        if (LanderSim* landerSim = dynamic_cast<LanderSim*>(simulator)) {
            landerSim->handleArrowKeyDown(key);
        }
    }
...
void SpecialKeyboardUpCB(int key, int x, int y) {
	...
	if (BaseSimulator* simulator = GlobalResourceManager::use()->getSimulator("landerSim")) {
        if (LanderSim* landerSim = dynamic_cast<LanderSim*>(simulator)) {
            landerSim->handleArrowKeyUp(key);  // New method needed in Lander class
        }
    }
